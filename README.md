...
1) 사용자 흐름 (MVP)
	...
	4.	상세 화면에 요약 정보만 표시 + [네이버로 열기](붙여넣기 원본으로 딥링크 실행)
...
3) 프런트 구조(RN+Web, 지도 미사용)
	...
	•	RouteDetail: 요약(거리·시간은 사용자가 입력한 값 or 빈값), 평가/태그/사진/댓글 + [네이버로 열기]
	•	My: 내 오토바이 선택, 내 활동
	•	컴포넌트
...
6) UX 디테일 (지도 없는 버전)
	...
	•	하단 고정 CTA: [네이버로 열기](문서 가이드대로 설치 여부/인앱브라우저 예외 처리)  ￼
	•	접근성/반응형
